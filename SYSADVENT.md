...
Nomad running in agent mode will receive requests for tasks from the server and, if possible, act on those requests. In the example above we asked Nomad to start 50 instances of the microbot task which in this example are webservers running in a Docker container as defined by the job definition. We asked Nomad to allocate ports for the containers we launched and monitor the health of those services to act on them should the health check ever fail.
...
To demonstrate a rolling updates, let's look at our job file `microbot.nomad` once again.
```
...
...
To verify that it is running on all hosts, let's run:
```
vagrant ssh nomad
